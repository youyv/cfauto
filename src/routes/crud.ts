...
ROUTES.set('GET /api/settings', async (req, env) => {
    ...
    if (type && !TEMPLATES[type]) return jsonError('Invalid template type: ' + type, 400);
...
ROUTES.set('POST /api/favorites', async (req, env) => {
    ...
    const { action, item } = await safeJson(req) as any;
...
ROUTES.set('GET /api/diag', async (_req, env) => {
    ...
    for (const k of keys) {
        try {
            ...
            results[k] = v === null ? '(not set)' : '(set, ' + v.length + ' bytes)';
