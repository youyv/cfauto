...
- **修复部署输出乱码与错位**：曾在 `deploy.bat` 里把 wrangler 输出重定向到文件再打印 —— cmd 会按控制台
  代码页（GBK）解释这些 UTF-8 字节，于是出现 `鈻?`、`鉀咃笍` 之类的乱码；改成只缓冲 stderr 之后，缓冲内容
  又在部署结束后才整段倒出，`[dns-fix]` 噪声与代理警告堆在结尾、顺序错乱。现在**完全不重定向** wrangler
  输出，让 Node 在真控制台下走 Unicode API 渲染；重试判定改回「任何失败都重试」，不再解析错误文本
  （错配成本只是多等 10 秒，远小于输出被搞乱的代价）。
- **dns-fix 默认静默**：该脚本被 NODE_OPTIONS 加载进**每一个** node 进程，而「本机 DNS 看起来正常、
  不干预」这类信息毫无信息量却每个进程都打一遍（一次部署刷 4 行）。现在这类「无动作」日志只在
