...
/**
 * 依赖缺失时给出可执行的指引，而不是甩一段 MODULE_NOT_FOUND 堆栈。
 * 全新克隆、还没装依赖就 build/deploy 是最常见的踩坑点（wrangler 的 [build] 钩子也会走到这里）。
 */
let esbuild;
try {
    esbuild = require('esbuild');
} catch (e) {
    console.error('❌ 缺少构建依赖 esbuild：' + (e && e.message ? e.message : String(e)));
    console.error('   请先安装依赖 —— Windows: 双击 install.bat；其他平台: pnpm install（或 npm install）');
    process.exit(1);
}
...
(async () => {
    ...
    // 静态资源 URL 的缓存键 = 版本号 + 内容指纹。
    // 只靠版本号时，改了代码但忘了升版本会让 /app.js?v=V12.0.0 原样不变，
    // 而它是 `max-age=31536000, immutable` —— 浏览器直接吃旧脚本，改动永远到不了用户面前。
    // 内容一变指纹就变，URL 随之改变，缓存自然失效（显示用版本号仍是 V<pkg.version>）。
    const assetVersion = 'V' + pkg.version + '.' + sha256(htmlContent + cssContent + jsContent + sweetalert2Content).slice(0, 8);
    const frontendBundle = [
        '// AUTO-GENERATED by build.js — DO NOT EDIT',
        'export const FRONTEND_VERSION = ' + JSON.stringify('V' + pkg.version) + ';',
        '/** 资源 URL 用（内容寻址）；显示用版本见 FRONTEND_VERSION */',
        'export const FRONTEND_ASSET_VERSION = ' + JSON.stringify(assetVersion) + ';',
