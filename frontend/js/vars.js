...
function addVarRow(t,k='',v=''){ const c=document.getElementById(`vars_${t}`); const d=document.createElement('div'); d.className=`flex gap-1 items-center mb-1 var-row-${t}`; let h=''; if(t==='cmliu'&&(k==='PROXYIP'||k==='DOH')){ const options=k==='DOH'?["https://dns.jhb.ovh/joeyblog","https://doh.cmliussss.com/CMLiussss","cloudflare-ech.com"]:ECH_PROXIES.flatMap(g=>g.list); h=`<select onchange="this.parentElement.querySelector('.val').value=this.value" class="w-4 border rounded text-[8px] bg-gray-50 cursor-pointer"><option>▼</option>${options.map(u=>`<option value="${u.split(' ')[0]}">${u}</option>`).join('')}</select>`; } d.innerHTML=`<input class="input-field w-1/4 key font-bold" placeholder="Key" value="${k}"><input class="input-field w-2/4 val" placeholder="Val" value="${v}"><label class="text-[9px] flex items-center gap-0.5"><input type="checkbox" class="secret-chk" onchange="this.parentElement.nextElementSibling.value=this.checked?'1':''" title="标记为Secret变量">🔒</label><input type="hidden" class="is-secret" value="">${h}<button onclick="removeVarRow(this,'${t}')" class="text-gray-300 hover:text-red-500 px-1 font-bold">×</button>`; c.appendChild(d); }
